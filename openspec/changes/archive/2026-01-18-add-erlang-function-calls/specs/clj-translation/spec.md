...
#### Scenario: Erlang function calls in nested contexts
- **WHEN** translating `(if (> (:erlang/unique_integer) 0) (:lists/empty? []) (:timer/sleep 100))`
- **THEN** all three Erlang function calls should be translated correctly
- **THEN** the conditional should evaluate based on the unique integer result